...
bool Ilator::Bootstrap(const std::string& root, bool opt) {
  ...
  if (vcd)  {
    ...
    fmt::format_to(vcdHeaderBuff,
        "$version Generated by ILAng (speak to Jack Toubes) $end\n"
        "$date {} $end\n"
        "$timescale 1ns $end\n"
    ...
    for (auto& state : absknob::GetSttTree(m_))
    {
      if (!(state->is_mem()))
      {

        if (inVcd.find(state) == inVcd.end())
        {
        ...
        fmt::format_to(vcdHeaderBuff, "  $var wire {} {} {} $end\n", 
              state->is_bool() ? 1 : state->sort()->bit_width(),
              inVcd[state],
              state->name().str());
      }
    }
    for (auto& input : absknob::GetInpTree(m_))
    {
      if (inVcd.find(input) == inVcd.end())
      {
        std::string newId = GetNewVcdIdentifier();
        inVcd[input] = newId;
      }
      ...
      fmt::format_to(vcdHeaderBuff, "  $var wire {} {} {} $end\n", 
            input->is_bool() ? 1 : input->sort()->bit_width(),
            inVcd[input],
              input->name().str());
    }
...
bool Ilator::GenerateIlaBasics(const std::string& dir) {
  ...
  auto PerIla = [this, &buff](const InstrLvlAbsCnstPtr& m) {
    ...
    if (this->vcd)
    {
      fmt::format_to(vcdHeaderBuff, "$scope function {} $end\n", valid_func->name);
    }
    ExprVarMap lut;
    ...
    if (this->vcd)
    {
      fmt::format_to(vcdHeaderBuff, "$upscope $end\n");
    }
      ...
      if (vcd)
      {
        if (curr->is_bool())
        {
          fmt::format_to(buff, "vcd_map[\"{id}\"] = ({var} ? \"1\" : \"0\");\n",
              fmt::arg("var", LookUp(next, lut)), 
              fmt::arg("id", inVcd[curr]));
        }
        else
        {
...
bool Ilator::GenerateExecuteKernel(const std::string& dir) {
  ...
  if (vcd)
  {
    fmt::format_to(buff, 
    "else {{\n"
    "  vcd_log << '#' << sc_time_stamp().to_default_time_units() << '\\n';\n"
    "}}\n");
  }
  ...
  // generate valid func call for each ILA
  auto PerIla = [this, &buff](const InstrLvlAbsCnstPtr& m) {

    ILA_NOT_NULL(m);
    fmt::format_to(
      buff, 
      "bool {valid_func_name}_ret = {valid_func_name}();\n",
  ...
  auto ExecInstr = [this, &buff](const InstrPtr& instr, bool child) {
    fmt::format_to(
        buff,
        "if ({valid_func_name}_ret && {decode_func_name}()) {{\n"
        "  {update_func_name}();\n"
        "  {child_counter}"
  ...
  {
    fmt::format_to(buff,
                       "for (auto pair : vcd_map) {{\n"
                       "  std::string spaces = (pair.second[0] == 'b') ? \"  \" : \"\";\n"
                       "  vcd_log << pair.second << spaces << pair.first << '\\n';\n"
                       "}}\n"
                       "vcd_map.clear();\n"
                       );
  }
...
bool Ilator::GenerateBuildSupport(const std::string& dir) {
  ...
  if (vcd)
  {
    fmt::format_to(vcdHeaderBuff, "$upscope $end\n"
    "$enddefinitions $end\n");
    WriteFile(os_portable_append_dir(os_portable_append_dir(dir, kDirSrc), "vcd_pre.txt"), vcdHeaderBuff);
  }
...
std::string Ilator::GetNewVcdIdentifier()
{
  static std::string lastIdentifier = {33};
  for (int i = 0; i < lastIdentifier.length(); i++)
  {
    lastIdentifier[i]++;
    if (lastIdentifier[i] == '"' || lastIdentifier[i] == '\\'
    || lastIdentifier[i] == '0' || lastIdentifier[i] == '1')
    {
      lastIdentifier[i]++;
    }
    
    if (lastIdentifier[i] > 126)
    {
      lastIdentifier[i] = 33;
      if (i == lastIdentifier.length() - 1)
      {
        lastIdentifier.append({33});
      }
    }
    else
    {
